...
with open("generated_data.txt", "w") as f:
    f.write("\nNames:\n")
    for _ in range(amounts["Names"]):
        name = dataFactory.getName()
        f.write(name + "\n")

    f.write("\nDates:\n")
    for _ in range(amounts["Dates"]):
        date = dataFactory.getDate()
        f.write(date + "\n")

    f.write("\nAddresses:\n")
    for _ in range(amounts["Addresses"]):
        address = dataFactory.getAddress()
        f.write(address + "\n")

    f.write("\nLicense Plates:\n")
    for _ in range(amounts["License Plates"]):
        license_plate = dataFactory.getLicensePlate()
        f.write(license_plate + "\n")

    f.write("\nEmails:\n")
    for _ in range(amounts["Emails"]):
        name = dataFactory.getName()
        email = dataFactory.getEmail(name)
        f.write(email + "\n")

    f.write("\nSocial Security Numbers:\n")
    for _ in range(amounts["Social Security Numbers"]):
        ssn = dataFactory.getSocialSecurityNumber()
        f.write(ssn + "\n")

    f.write("\nLicense Numbers:\n")
    for _ in range(amounts["License Numbers"]):
        license_number = dataFactory.getLicenseNumber()
        f.write(license_number + "\n")

    f.write("\nBusiness Types:\n")
    for _ in range(amounts["Business Types"]):
        business_type = dataFactory.getBusinessType()
        f.write(business_type + "\n")

    f.write("\nContent:\n")
    content = dataFactory.getContent(amounts["Content"])
    f.write(content + "\n")

    f.write("\nLatitude and Longitude:\n")
    for _ in range(amounts["Latitude and Longitude"]):
        latitude = dataFactory.addressDataValues.getLatitude()
        longitude = dataFactory.addressDataValues.getLongitude()
        f.write(f"Latitude: {latitude}, Longitude: {longitude}\n")


name_data_list = []
date_data_list = []
social_numbers_data_list = []

with open("data_objects.py", "w") as file:
    for _ in range(amounts["Names"]):
        name = dataFactory.getName()
        name_data_list.append(name)
    ...
    for _ in range(amounts["Social Security Numbers"]):
        ssn = dataFactory.getSocialSecurityNumber()
        social_numbers_data_list.append(ssn)

    file.write(f"NAMES = {name_data_list}\n")
    file.write(f"DATES = {date_data_list}\n")
    file.write(f"SSN = {social_numbers_data_list}\n") 
